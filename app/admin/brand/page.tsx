...
export default async function AdminBrandPage({ searchParams }: { searchParams?: SearchParams }) {
  ...
  return (
    <main className="mx-auto min-h-screen max-w-7xl px-4 py-8 md:px-8">
      ...
      {saved ? <div className="mt-5 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm font-bold text-emerald-800"><p>Brand settings saved.</p><p>Public site will use these values.</p>{!config.hasBlob ? <p>Asset upload fields are saved as URLs but uploads require Blob token.</p> : null}</div> : null}
