...
function AssetField({ name, label, usedWhere, value, uploadEnabled, meta }: { name: string; label: string; usedWhere: string; value: string; uploadEnabled: boolean; meta?: Record<string, unknown> }) {
  ...
  return <div className="grid gap-2 border-b border-slate-100 px-3 py-2 text-xs font-bold text-slate-600 lg:grid-cols-[150px_64px_1fr_180px_150px_80px_90px] lg:items-center">
    ...
    <span className="w-fit rounded-full border border-slate-200 bg-white px-2 py-0.5 text-[10px] font-black uppercase">{status}</span>
  ...
  return <main className="mx-auto min-h-screen max-w-7xl px-4 py-5 md:px-6">
    ...
    <form action={saveBrandSettingsAction} className="mt-4 grid gap-4 lg:grid-cols-[1fr_340px]">
      <div className="grid gap-4">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-soft"><div className="flex items-center justify-between"><h2 className="font-black text-slate-950">Brand assets</h2><span className={`rounded-full border px-2 py-1 text-[10px] font-black uppercase ${config.hasBlob ? "border-emerald-200 text-emerald-700" : "border-amber-200 text-amber-700"}`}>Blob {config.hasBlob ? "enabled" : "disabled"}</span></div><p className="mt-1 text-xs font-bold text-slate-500">PNG/JPEG/WebP only; SVG upload disabled. Manual URLs work even when Blob uploads are disabled.</p><div className="mt-3 overflow-hidden rounded-xl border border-slate-100"><div className="hidden bg-slate-50 px-3 py-2 text-[10px] font-black uppercase tracking-[0.12em] text-slate-400 lg:grid lg:grid-cols-[150px_64px_1fr_180px_150px_80px_90px]"><div>Asset</div><div>Preview</div><div>URL</div><div>Upload</div><div>Used in</div><div>Status</div><div>Reset</div></div>{ASSETS.map(([name, label, used]) => <AssetField key={name} name={name} label={label} usedWhere={used} value={String(settings[name] || "")} uploadEnabled={config.hasBlob} meta={settings.assetMetadata?.[name]} />)}</div></section>
      ...
      <aside className="sticky top-20 grid gap-3 self-start rounded-2xl border border-slate-200 bg-white p-4 shadow-soft"><h2 className="font-black text-slate-950">Public previews</h2><div className="rounded-xl border border-slate-100 bg-slate-50 p-3"><div className="text-[10px] font-black uppercase text-slate-400">Site header</div><div className="mt-2 flex items-center gap-2">{settings.headerLogo || settings.primaryLogo ? <img src={settings.headerLogo || settings.primaryLogo} alt="" className="h-8 w-8 object-contain" /> : null}<span className="text-xl font-black tracking-[-0.04em]">{settings.siteName}</span></div></div><div className="rounded-xl border border-slate-100 bg-slate-50 p-3"><div className="text-[10px] font-black uppercase text-slate-400">Hero</div><h3 className="mt-1 text-lg font-black tracking-[-0.05em]">{settings.mainSlogan}</h3><p className="mt-1 text-sm font-bold text-slate-500">{settings.heroSubtitle}</p></div><div className="rounded-xl border border-slate-100 bg-slate-50 p-3 text-sm font-bold text-slate-600">Share footer: {settings.cardFooterText} · {settings.createdWithText}</div><div className="rounded-xl border border-slate-100 bg-slate-50 p-3"><div className="text-[10px] font-black uppercase text-slate-400">Favicon / X / watermark</div><div className="mt-2 flex items-center gap-2">{[settings.favicon, settings.xAvatar, settings.watermarkMark].map((src, i) => <div key={i} className="h-10 w-10 rounded-lg border bg-white">{src ? <img src={src} alt="" className="h-full w-full object-contain" /> : null}</div>)}</div>{settings.xBanner ? <img src={settings.xBanner} alt="" className="mt-2 h-20 w-full rounded-xl object-cover" /> : <div className="mt-2 h-20 rounded-xl bg-slate-200" />}</div></aside>
    </form>
  </main>;
}
