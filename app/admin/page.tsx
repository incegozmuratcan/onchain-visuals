  ...
  return (
    <main className="mx-auto min-h-screen max-w-7xl px-4 py-4 md:px-6">
      ...
      <section className="mt-3 grid gap-2 md:grid-cols-5">
        ...
        <form action={bulkRefreshCoinGeckoLogosAction}><button className="w-full rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs font-black">Bulk refresh CoinGecko logos</button></form><form action={scanMetricLogosAction}><button className="w-full rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs font-black">Scan metric logos</button></form>
      </section>
      ...
      <section className="mt-3 grid gap-3 lg:grid-cols-[1fr_420px]">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-3 shadow-soft"><h2 className="text-sm font-black text-slate-950">Latest bulk results</h2>{[summaries.coingecko, summaries.coinmarketcap].filter(Boolean).map((summary) => summary ? <div key={summary.provider} className="mt-3 rounded-lg border border-slate-100 bg-slate-50 p-2 text-xs font-bold text-slate-700"><div className="font-black text-slate-950">{summary.provider}</div><div>{summary.refreshed} fetched · {summary.autoApproved ?? 0} auto-approved · {summary.candidates ?? 0} candidates · {summary.missingMappings} missing mappings · {summary.errors} errors</div><div className="text-xs text-slate-400">{new Date(summary.timestamp).toLocaleString()}</div>{summary.firstSkippedReasons?.length ? <div className="mt-1 text-xs text-amber-800">Skipped: {summary.firstSkippedReasons.slice(0, 3).join("; ")}</div> : null}{summary.firstErrors.length ? <div className="mt-1 text-xs text-red-700">Errors: {summary.firstErrors.join("; ")}</div> : null}</div> : null)}<Link href="/admin/logos?filter=issues" className="mt-4 block rounded-2xl border border-slate-200 px-4 py-3 text-center text-sm font-black">Run logo health check</Link></div>
      </section>
    </main>
  );
}
