...
import { AdminShell, AdminStatusPill, AdminToolbar } from "@/components/admin/AdminPrimitives";
...
function IssueBadge({ issue }: { issue: string }) {
  const tone = issue === "visual_rejected" ? "red" : issue.includes("missing") || issue.includes("failed") || issue.includes("rejected") ? "amber" : "gray";
  return <AdminStatusPill tone={tone}>{issue.replaceAll("_", " ")}</AdminStatusPill>;
}
...
function MetricScanSummary({ scanSummary }: { scanSummary: NonNullable<ReturnType<typeof parseMetricLogoScanSummary>> }) {
  ...
  return <div className="rounded-lg border border-slate-100 bg-slate-50 p-2 text-xs font-bold text-slate-700"><div className="flex items-center justify-between gap-2"><div className="font-black">Last metric logo scan</div><div className="text-slate-400">{new Date(scanSummary.timestamp).toLocaleString()}</div></div><div className="mt-1">{scanSummary.metricsScanned} metrics · {scanSummary.rowsChecked} entities · {scanSummary.newEntities} new · {scanSummary.missingCoinGeckoIds} missing CG · {scanSummary.errors.length} error{scanSummary.errors.length === 1 ? "" : "s"}</div><div className="mt-2 grid gap-1">{groups.map(([label, details]) => details.length ? <details key={label}><summary className="cursor-pointer">Show {label.toLowerCase()} ({details.length})</summary><div className="mt-1 max-h-40 overflow-auto rounded-lg bg-white p-2 text-slate-700">{details.map((detail) => <Link key={`${label}-${detail.slug}-${detail.actionTaken}`} href={`/admin/logos/${detail.slug}`} className="mr-2 inline-block underline" title={detail.reason || detail.error || ""}>{detail.name}</Link>)}</div></details> : null)}</div></div>;
}
...
export default async function AdminLogosPage({ searchParams }: { searchParams?: SearchParams }) {
  ...
  return (
    <AdminShell active="logos" title="Logo Manager" subtitle="Dense source QA and DB-approved logo operations.">
      ...
      <section className="mt-3 rounded-xl border border-slate-200 bg-white p-3 shadow-soft">
        <div className="flex flex-wrap items-center justify-between gap-2"><h2 className="text-sm font-black tracking-[-0.02em] text-slate-950">Logo QA Inbox</h2><p className="text-xs font-bold text-slate-400">Total logos: {counts.all} · Approved: {counts.approved} · Needs action: {needsAction}</p></div>
      ...
      <AdminToolbar className="mt-2">
        <div className="grid gap-2 lg:grid-cols-[1fr_auto_auto] lg:items-start">
          <form className="grid gap-2 md:grid-cols-[1fr_170px_auto]"><input name="q" defaultValue={query} className="rounded-lg border border-slate-200 px-3 py-2 text-sm" placeholder="Search name, slug, category, provider ID, issue" /><input type="hidden" name="filter" value={filter} /><select name="sort" defaultValue={sort} className="rounded-lg border border-slate-200 px-3 py-2 text-sm"><option value="name">Name A-Z</option><option value="issues">Issues first</option><option value="status">Status</option><option value="category">Category</option><option value="provider">Source provider</option><option value="updated">Last updated</option><option value="fetch">Last fetch time</option></select><button className="rounded-lg bg-slate-950 px-4 py-2 text-sm font-black text-white">Search</button></form>
          <details className="rounded-lg border border-slate-200 bg-white px-3 py-2"><summary className="cursor-pointer text-sm font-black text-slate-950">▸ Add logo</summary><form action={createLogoAction} className="mt-2 grid gap-2 md:grid-cols-[220px_110px_auto]"><input name="name" className="rounded-lg border border-slate-200 px-3 py-2 text-sm" placeholder="Name" required /><select name="category" className="rounded-lg border border-slate-200 px-3 py-2 text-sm"><option>project</option><option>chain</option><option>asset</option></select><button className="rounded-lg bg-slate-950 px-4 py-2 text-sm font-black text-white">Create</button></form></details>
          <details className="rounded-lg border border-slate-200 bg-white px-3 py-2"><summary className="cursor-pointer text-sm font-black text-slate-950">Source tools</summary><div className="mt-2 flex max-w-3xl flex-wrap gap-2"><form action={bulkRefreshCoinGeckoLogosAction}><input type="hidden" name="mode" value="smart" /><button className="rounded-full bg-slate-950 px-3 py-1.5 text-xs font-black text-white">Refresh missing</button></form><form action={bulkRefreshCoinGeckoLogosAction}><input type="hidden" name="mode" value="retry-errors" /><button className="rounded-full border border-slate-200 px-3 py-1.5 text-xs font-black">Retry failed</button></form><form action={bulkRefreshCoinGeckoLogosAction}><input type="hidden" name="mode" value="force-all" /><button className="rounded-full border border-slate-200 px-3 py-1.5 text-xs font-black">Force all</button></form><form action={applySafeCoinGeckoCandidatesAction}><button className="rounded-full border border-slate-200 px-3 py-1.5 text-xs font-black">Apply safe CG</button></form><form action={scanMetricLogosAction}><button className="rounded-full border border-slate-200 px-3 py-1.5 text-xs font-black">Scan metrics</button></form><form action={bulkRefreshCoinMarketCapLogosAction}><button disabled={!process.env.COINMARKETCAP_API_KEY} className="rounded-full border border-slate-200 px-3 py-1.5 text-xs font-black disabled:opacity-50">Refresh CMC</button></form></div><div className="mt-2 grid gap-2">{scanSummary ? <MetricScanSummary scanSummary={scanSummary} /> : null}{[summaries.coingecko, summaries.coinmarketcap].filter(Boolean).map((summary) => summary ? <CompactRefreshSummary key={summary.provider} summary={summary} missingMappings={summary.provider === "CoinGecko" ? missingMappingRows : undefined} /> : null)}</div></details>
        </div>
      </AdminToolbar>

      <section className="mt-2 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-soft">
        <div className="hidden grid-cols-[1.25fr_82px_145px_95px_1fr_140px_150px] gap-2 border-b border-slate-100 bg-slate-50 px-3 py-2 text-[10px] font-black uppercase tracking-[0.12em] text-slate-400 lg:grid"><div>Logo</div><div>Category</div><div>Provider IDs</div><div>Status</div><div>Providers/issues</div><div>Last fetch</div><div>Recommended action</div></div>
        {rows.map((row) => {
          const preview = row.logo.approved_logo_url || row.logo.fallback_logo_url;
          const displayIssues = row.issues.filter((issue) => issue !== "upload_disabled" && issue !== "already_approved" && issue !== "coingecko_auto_approved").slice(0, 2);
          return (
            <Link key={row.logo.id} href={`/admin/logos/${row.logo.slug}`} className="grid gap-2 border-b border-slate-100 px-3 py-2 transition hover:bg-slate-50 lg:min-h-[48px] lg:grid-cols-[1.25fr_82px_145px_95px_1fr_140px_150px] lg:items-center">
              <div className="flex min-w-0 items-center gap-2">{preview ? <img src={preview} alt="" className="h-7 w-7 rounded-full border border-slate-200 bg-white object-contain" /> : <div className="h-7 w-7 rounded-full bg-slate-100" />}<div className="min-w-0"><div className="truncate text-sm font-black text-slate-950">{row.logo.name}</div><div className="truncate text-[11px] font-bold text-slate-400">{row.logo.slug}{!row.logo.approved_logo_url && row.logo.fallback_logo_url ? " · fallback" : ""}</div></div></div>
              <div className="text-xs font-black text-slate-600">{row.logo.category}</div>
              <div className="text-[11px] font-bold leading-4 text-slate-500"><div>CG: {row.coinGeckoId || "missing"}</div><div>CMC: {row.coinMarketCapId || "missing"}</div></div>
              <StatusBadge status={row.logo.status} />
              <div className="min-w-0"><div className="truncate text-[11px] font-bold text-slate-400">{row.providerSummary}</div><div className="mt-0.5 flex flex-wrap gap-1">{displayIssues.length ? displayIssues.map((issue) => <IssueBadge key={issue} issue={issue} />) : <IssueBadge issue="approved" />}</div></div>
              <div className="truncate text-[11px] font-bold text-slate-500" title={row.logo.last_fetch_error || ""}>{row.logo.last_fetch_provider || "—"} · {row.logo.last_fetch_at ? new Date(row.logo.last_fetch_at).toLocaleDateString() : "never"}</div>
              <div className="text-xs font-bold text-slate-600">{row.recommendedAction}</div>
            </Link>
          );
        })}
