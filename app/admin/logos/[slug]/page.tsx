  ...
  return (
    <main className="mx-auto min-h-screen max-w-6xl px-4 py-8 md:px-8">
      ...
      <header className="mt-5 grid gap-5 rounded-[32px] border border-slate-200 bg-white p-6 shadow-soft md:grid-cols-[1fr_140px] md:items-center">
        <div><p className="text-xs font-black uppercase tracking-[0.22em] text-slate-400">{logo.category} · {logo.status}</p><h1 className="mt-2 text-5xl font-black tracking-[-0.07em] text-slate-950">{logo.name}</h1><p className="mt-2 text-sm font-bold text-slate-500">{logo.slug}</p></div>
        <div className="flex h-28 w-28 items-center justify-center overflow-hidden rounded-full border border-slate-200 bg-slate-50">{logo.approved_logo_url ? <img src={logo.approved_logo_url} alt={`${logo.name} approved logo`} className="h-full w-full object-contain" /> : <span className="text-xs font-black text-slate-400">No approved logo</span>}</div>
      </header>

      <section className="mt-6 grid gap-4 lg:grid-cols-2">
        <form action={addCoinGeckoAction} className="rounded-[26px] border border-slate-200 bg-white p-5 shadow-soft">{hidden}<h2 className="font-black text-slate-950">CoinGecko candidate</h2><input name="coinGeckoId" className="mt-3 w-full rounded-2xl border border-slate-200 px-4 py-3" placeholder="coingecko coin id, e.g. ethereum" required /><button className="mt-3 rounded-2xl bg-slate-950 px-5 py-3 font-black text-white">Fetch CoinGecko</button></form>
      ...
      <section className="mt-6 grid gap-4">
        ...
        {sources.map((source) => (
          <article key={source.id} className="grid gap-4 rounded-[26px] border border-slate-200 bg-white p-5 shadow-soft md:grid-cols-[96px_1fr_220px] md:items-center">
            <div className="flex h-20 w-20 items-center justify-center overflow-hidden rounded-full border border-slate-200 bg-slate-50"><img src={source.blob_url || source.image_url} alt="" className="h-full w-full object-contain" /></div>
            <div className="min-w-0"><div className="font-black text-slate-950">{source.provider} · {source.status}</div><a className="mt-1 block truncate text-sm font-bold text-slate-500 underline" href={source.image_url}>{source.image_url}</a>{source.rejection_reason ? <p className="mt-2 text-sm font-bold text-red-600">{source.rejection_reason}</p> : null}</div>
