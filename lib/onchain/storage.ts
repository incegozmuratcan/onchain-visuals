...
export async function loadSourceRuns() { if (hasDatabaseConfig()) { const res = await query(`select * from onchain_source_runs order by started_at desc limit 500`); return res.rows.map((r:any)=>({ id:r.id, source:r.source, datasetSlug:r.dataset_slug, status:r.status, startedAt:r.started_at, finishedAt:r.finished_at, errorMessage:r.error_message, rowsFetched:r.rows_fetched, payloadHash:r.payload_hash, metadata:r.metadata_json || {} })) as SourceRun[]; } return readJson<SourceRun[]>(runsFile, []); }
