...
test('snapshot contract includes required statuses and stale fallback', () => { ['source_config_required','source_error','stale'].forEach((s)=>assert.ok(snapshots.includes(s))); assert.ok(snapshots.includes("status:'fresh'") || snapshots.includes("status: 'fresh'")); assert.ok(snapshots.includes('loadLatestChartSnapshot')); assert.ok(snapshots.includes('markSnapshotStale')); });
