...
const datasets = await readFile(
  new URL("../lib/datasets.ts", import.meta.url),
  "utf8",
);
const homeClient = await readFile(
  new URL("../components/HomeClient.tsx", import.meta.url),
  "utf8",
);
const datasetLibrary = await readFile(
  new URL("../components/DatasetLibrary.tsx", import.meta.url),
  "utf8",
);
...
const promptPanel = await readFile(
  new URL("../components/PromptPanel.tsx", import.meta.url),
  "utf8",
);
const chainApi = await readFile(
  new URL("../app/api/chain-revenue/route.ts", import.meta.url),
  "utf8",
);
...
test("home defaults to BTC ETF Daily with context-aware BTC ETF settings", () => {
  ...
  assert.match(
    homeClient,
    /useState<string\[\]>\(\["Capital Flows", "BTC ETF", "Daily"\]\)/,
  );
  ...
  assert.doesNotMatch(
    promptPanel.match(/detected\.isBtcEtf \? \([\s\S]*?\) : \(/)?.[0] || "",
    /Top \{limit\}|Select result count|Revenue|Chains/,
  );
...
test("Capital Flows appears first and public dataset library exposes only BTC ETF there", () => {
  assert.ok(
    datasets.indexOf('id: "capital_flows"') < datasets.indexOf('id: "chains"'),
  );
  assert.ok(
    datasets.indexOf('id: "chains"') < datasets.indexOf('id: "protocols"'),
  );
  assert.ok(
    datasets.indexOf('id: "protocols"') < datasets.indexOf('id: "assets"'),
  );
  assert.ok(
    datasets.indexOf('id: "assets"') < datasets.indexOf('id: "infrastructure"'),
  );
  ...
  assert.doesNotMatch(
    publicCapitalFlows,
    /ETH ETF|BTC vs ETH|Whale|Liquidation|config/i,
  );
});
...
test("BTC ETF presets appear only under Capital Flows in public menus and API search still accepts all periods", () => {
  ...
  assert.doesNotMatch(
    nonCapital,
    /BTC ETF Daily Flowboard|BTC ETF Weekly Flowboard|BTC ETF Monthly Flow Report/,
  );
  assert.match(homeClient, /"BTC ETF Daily Flowboard"/);
  assert.doesNotMatch(
    homeClient,
    /"BTC ETF Weekly Flowboard"[\s\S]*tryCards|"BTC ETF Monthly Flow Report"[\s\S]*tryCards/,
  );
  assert.match(chainApi, /monthly\|issuer\\s\+report/);
  assert.match(chainApi, /weekly\|week/);
  assert.match(
    chainApi,
    /buildChartSnapshot\("btc-etf-flowboard", btcEtfPeriod\)/,
  );
...
test("BTC ETF completed-row builders power daily weekly monthly cards without awkward missing-issuer strings", () => {
  ...
  assert.match(
    snapshots,
    /getLatestCompletedBtcEtfDays\(flowResult\.data\.rows, 5\)/,
  );
  assert.doesNotMatch(snapshots, /No issuer had/);
  assert.match(
    snapshots,
    /d.slug === ["\']btc-etf-flowboard["\'][\s\S]*["\']Farside["\']/,
  );
});
...
test("public BTC ETF card has clean static export UI and source footer behavior", () => {
  ...
  assert.match(
    shareCard,
    /data.datasetSlug === "btc-etf-flowboard"[\s\S]*"Farside"/,
  );
  assert.doesNotMatch(
    shareCard,
    /ExportFormatSelector|1600x900|1080x1350|<Tooltip\b|overflow-x-auto/,
  );
