...
const farside = await importSource("../lib/onchain/sources/farside.ts", {
  "import { fetchText } from './http';": "const fetchText = async()=>'';",
});
const defillama = await importSource("../lib/onchain/sources/defillama.ts", {
  "import { fetchJson } from './http';": "const fetchJson = async()=>({});",
});
const registry = await readFile(
  new URL("../lib/onchain/registry.ts", import.meta.url),
  "utf8",
);
...
const client = await readFile(
  new URL("../components/onchain/DatasetChartClient.tsx", import.meta.url),
  "utf8",
);
const defillamaSource = await readFile(
  new URL("../lib/onchain/sources/defillama.ts", import.meta.url),
  "utf8",
);
const templates = await Promise.all(
  [
    "../components/onchain/templates/LeaderboardBarChart.tsx",
    "../components/onchain/templates/EtfFlowboard.tsx",
    "../components/onchain/templates/BtcEthEtfComparison.tsx",
    "../components/onchain/templates/SpecialBoards.tsx",
...
test("Farside parser handles BTC and ETH fixtures with tolerant headers", () => {
  ...
  assert.equal(
    btc.find((r) => r.issuer === "Grayscale GBTC").flowUsd,
    -2_500_000,
  );
...
test("chain fee normalization excludes protocol-only entities and Off Chain", () => {
  const rows = defillama.normalizeChainFeesFromBreakdown({
    totalDataChartBreakdown: [
      ...
      [86500, { Solana: 50, Base: 40, Grayscale: 7 }],
    ],
  });
...
test("chain and protocol builders use different DefiLlama methods", () => {
  assert.match(
    defillamaSource,
    /fetchChainRevenue[\s\S]*dataType=dailyFees[\s\S]*fetchProtocolRevenue[\s\S]*dataType=dailyRevenue/,
  );
...
test("static export preview has exact dimensions and responsive frame", () => {
  for (const fmt of [
    ...
    assert.match(chartShell, new RegExp(fmt));
...
test("non-active snapshots render state cards before chart templates", () => {
  ...
  assert.match(
    client,
    /source_config_required[\s\S]*SourceConfigRequiredState/,
  );
...
test("registry keeps config/disabled datasets out of active promotion", () => {
  for (const slug of [
    "stablecoin-net-transfers-by-chain",
    "monthly-unlock-watch",
    "large-holders-board",
    "whale-transfers",
    "binance-liquidation-pulse",
    "digital-asset-treasuries",
  ]) {
    const block =
      registry.match(
        new RegExp(`slug:'${slug}'[\\s\\S]*?supportedPeriods:\[[^\]]+\]`),
      )?.[0] || "";
...
test("stablecoin values and no generic empty chart policy are encoded", () => {
  ...
  assert.match(
    chartShell + client,
    /No fake data is emitted|No fake data is shown/,
  );
});
