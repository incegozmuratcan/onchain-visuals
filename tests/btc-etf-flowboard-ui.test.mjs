...
const chartShell = await readFile(
  new URL("../components/onchain/ChartShell.tsx", import.meta.url),
  "utf8",
);
const etfFlowboard = await readFile(
  new URL("../components/onchain/templates/EtfFlowboard.tsx", import.meta.url),
  "utf8",
);
const shareCard = await readFile(
  new URL("../components/ShareCard.tsx", import.meta.url),
  "utf8",
);
const snapshots = await readFile(
  new URL("../lib/onchain/snapshots.ts", import.meta.url),
  "utf8",
);
...
async function importSnapshotBuilders() {
  ...
  source = source
    ...
    .replace(
      ...
      "const cumulative_sum = (values)=>values.reduce((out,value,index)=>(out.push((out[index-1] || 0) + value), out), []); const rolling_20d = (values)=>values.slice(-20).reduce((a,b)=>a+b,0); const streak_count = (values)=>values.length; const long_short_imbalance = ()=>null; const marketShares = ()=>[]; const safeChangePct = ()=>null; const supply_pressure_score = ()=>null;",
    )
    .replace(
      /import \{[\s\S]*?fetchChainRevenue[\s\S]*?\} from ["']\.\/sources\/defillama["'];/,
      "const fetchChainRevenue = async()=>({ok:false}); const fetchStablecoinSupplyByChain = async()=>({ok:false}); const fetchDexVolumeByChain = async()=>({ok:false}); const fetchProtocolRevenue = async()=>({ok:false}); const fetchDexProtocols = async()=>({ok:false}); const fetchPerpProtocols = async()=>({ok:false}); const fetchCexTransparency = async()=>({ok:false});",
    )
    .replace(
      /import \{[\s\S]*?fetchBtcEtfFlows[\s\S]*?\} from ["']\.\/sources\/farside["'];/,
      "const fetchBtcEtfFlows = async()=>({ok:false}); const fetchEthEtfFlows = async()=>({ok:false});",
    )
    .replace(
      /import \{ fetchStablecoinNetTransfersLatest \} from ["']\.\/sources\/dune["'];/,
      "const fetchStablecoinNetTransfersLatest = async()=>({ok:false});",
    )
    .replace(
      /import \{ largeHolderMissingConfig \} from ["']\.\/sources\/etherscan["'];/,
      "const largeHolderMissingConfig = ()=>[];",
    )
    .replace(
      /import \{ whaleTransferMissingConfig \} from ["']\.\/sources\/whaleAlert["'];/,
      "const whaleTransferMissingConfig = ()=>[];",
    )
    .replace(
      /import \{[\s\S]*?loadLatestChartSnapshot[\s\S]*?\} from ["']\.\/storage["'];/,
      "const loadLatestChartSnapshot = async()=>null; const markSnapshotStale = (snapshot)=>snapshot; const saveChartSnapshot = async()=>{}; const sourceRun = async()=>{};",
    );
  ...
  assert.doesNotMatch(
    js,
    /from [\"']\.{1,2}\//,
    "snapshot builder fixture must be self-contained before importing",
  );
...
const btcDataset = {
  id: "btc-etf-flowboard",
  slug: "btc-etf-flowboard",
  name: "BTC ETF Flowboard",
  description: "BTC ETF flows",
  category: "capital-flows",
  status: "active",
  requiredEnv: [],
  primarySource: "farside",
...
const btcFlowResult = {
  ...
  data: {
    ...
    rows: [
      ...
      ...issuers.map(([issuer, ticker, flowUsd]) => ({
        date,
        asset: "BTC",
        issuer,
        ticker,
...
function functionBlock(name) {
  ...
  const candidates = ["\nexport function ", "\nasync function "]
    .map((marker) => snapshots.indexOf(marker, start + 1))
...
test("BTC ETF visual avoids tooltip artifacts, scroll classes, and horizontal overflow patterns", () => {
  ...
  assert.equal(
    /overflow-(x-|y-)?auto|overflow-scroll/.test(etfFlowboard),
    false,
  );
...
test("Daily BTC ETF snapshot is latest-completed-day first with only Top Driver and Since Launch support", () => {
  ...
  assert.match(
    dailyBlock,
    /getTopIssuerContributors\(summary\.issuerLatest, 5\)/,
  );
  ...
  assert.doesNotMatch(
    etfFlowboard,
    /5-day context|Top issuer contributors|Latest completed row/,
  );
});
...
test("Weekly BTC ETF snapshot elevates weekly net flow and renders signed zero-baseline bars", () => {
  ...
  assert.doesNotMatch(
    weeklyBlock,
    /Latest completed row|Last five completed sessions|Cumulative Flow/,
  );
  assert.doesNotMatch(
    weeklyBlock,
    /Best Inflow Day|Worst Outflow Day|Top Issuer This Week/,
  );
...
test("Monthly BTC ETF card is an MTD flow report with compact issuer summary", () => {
  ...
  assert.match(
    monthlyBlock,
    /monthlyVisual:[\s\S]*["\']mtd_signed_daily_flow_chart["\']/,
  );
  assert.match(monthlyBlock, /Largest Inflow Day|Smallest Outflow/);
  assert.match(monthlyBlock, /Largest Outflow Day/);
  assert.match(monthlyBlock, /latestCompletedDay/);
  assert.match(monthlyBlock, /["\']Since Launch["\']/);
  assert.doesNotMatch(
    monthlyBlock,
    /Cumulative Flow|Current month-to-date Bitcoin ETF issuer flows using completed Farside rows|issuer_leaderboard/,
  );
...
test("BTC ETF snapshot builders emit semantic card contracts for public daily weekly and monthly views", () => {
  const daily = snapshotBuilders.buildBtcEtfDailyCard(
    btcDataset,
    "daily",
    btcFlowResult,
  );
  ...
  assert.equal(
    daily.series.tables.some((row) => Number(row.value) === 0),
    false,
  );
  ...
  assert.equal(
    JSON.stringify(daily.series.cards).includes("Issuer breadth"),
    false,
  );

  const weekly = snapshotBuilders.buildBtcEtfWeeklyCard(
    btcDataset,
    "weekly",
    btcFlowResult,
  );
  ...
  assert.equal(weekly.sourceLabel, "Farside");
  assert.ok(
    weekly.headlineMetrics.some((metric) => metric.label === "Since Launch"),
  );
  assert.equal(weekly.series.lines.length, 0);
  assert.equal(weekly.series.bars.length, 5);
  assert.ok(
    weekly.series.bars.some((bar) => bar.value < 0),
    "weekly outflows stay negative for zero-baseline rendering",
  );
  for (const bar of weekly.series.bars) {
    assert.equal(bar.magnitude, Math.abs(bar.value));
    assert.equal(bar.value < 0, bar.sign === "negative");
    assert.match(bar.valueLabel, /^[+-]/);
    assert.match(bar.sign, /^(positive|negative)$/);
  }
  assert.ok(weekly.series.tables.length <= 5);
  assert.equal(
    weekly.series.tables.some((row) => Number(row.value) === 0),
    false,
  );
  assert.equal(JSON.stringify(weekly).includes("Cumulative Flow"), false);
  assert.equal(
    JSON.stringify(weekly).includes("Last five completed sessions"),
    false,
  );

  const monthly = snapshotBuilders.buildBtcEtfMonthlyIssuerCard(
    btcDataset,
    "monthly",
    btcFlowResult,
  );
  ...
  assert.equal(monthly.headlineMetrics[2].label, "Largest Outflow Day");
  ...
  assert.equal(monthly.sourceLabel, "Farside");
  ...
  assert.ok(
    monthly.series.bars.some((bar) => bar.value < 0),
    "monthly outflows stay negative for zero-baseline rendering",
  );
  assert.ok(monthly.series.bars.every((bar) => bar.valueLabel));
  assert.equal(monthly.series.lines.length, 0);
  assert.ok(
    monthly.series.bars.some((bar) => bar.value > 0),
    "monthly inflows stay positive for zero-baseline rendering",
  );
  assert.ok(monthly.series.bars.filter((bar) => bar.showLabel).length <= 3);
  assert.ok(
    monthly.headlineMetrics.some((metric) => metric.label === "Since Launch"),
  );
  assert.ok(monthly.series.tables.length <= 5);
  assert.equal(
    monthly.series.tables.some((row) => Number(row.value) === 0),
    false,
  );
