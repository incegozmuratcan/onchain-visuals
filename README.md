...
- Logo detail pages defensively render malformed metadata, missing approved sources, invalid image URLs and local-vault candidates without client-side crashes. Local vault manifest entries appear as importable/usable sources; approved vault assets can be imported as the approved local-vault source only when they are not visually rejected/fallback-preferred and no stronger admin-approved manual/upload source exists.
...
- Public hero logo alignment can be tuned from Brand Settings with horizontal offset, max width and spacing controls. The public hero centers the uploaded wordmark without duplicating the site name and uses calmer slogan typography.
