...
const ACTION_ISSUES = new Set([
  "missing_approved_logo", "missing_coingecko_id", "missing_cmc_id", "missing_defillama_source", "coingecko_id_needs_review", "coingecko_fetch_failed", "cmc_fetch_failed", "defillama_no_reliable_source", "visual_rejected", "unsafe_migrated_candidate", "needs_review", "newly_discovered_entity", "metric_scan_error", "coingecko_candidate_waiting", "metric_scan_missing_coingecko_id", "metric_scan_candidate_added", "auto_approve_skipped", "db_overlay_not_applied", "rejected_source",
]);
...
function rowMatchesFilter(row: LogoResultRow, filter: string) {
  ...
  if (filter === "provider_errors") return row.issues.some((issue) => issue === "coingecko_fetch_failed" || issue === "cmc_fetch_failed" || issue === "defillama_no_reliable_source" || issue === "coingecko_id_needs_review");
...
export function LogoResultsClient({ rows, initialQuery = "", defaultLimit = PAGE_SIZE, activeFilter = "issues" }: { rows: LogoResultRow[]; initialQuery?: string; defaultLimit?: number; activeFilter?: string }) {
  ...
  return <div>
    <div className="border-b border-slate-100 p-2">
      <input value={query} onChange={(event) => { setQuery(event.target.value); setLimit(defaultLimit); }} className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm outline-none transition focus:border-slate-400" placeholder="Type to filter name, slug, category, CG/CMC/DefiLlama, source, coverage, issue" />
