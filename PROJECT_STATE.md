...
- Added deterministic DefiLlama validation coverage for ENI llamao chain icon, Provenance llamao chain icon, and BSV token icon (`token-icons.llamao.fi/.../bitcoin-cash-sv?w=48&h=48`).
- Fixed provider-save target identity by preferring explicit logo slug lookup in form actions before name/category upsert; this prevents duplicate alias-family persistence drift (notably `io-net` vs `ionet`).
- CoinGecko helper now supports explicit admin override for low-confidence/non-recommended visible candidates via **Use as review**; saved rows are review candidates (`status=candidate`, `metadata.reviewStatus=needs_review`, `metadata.approvalOrigin=admin_selected_low_confidence`) and are not auto-approved.
...
- DefiLlama Use + Fetch now submits and persists the exact preview candidate payload (source URL/image URL/source type/image pattern + token metadata) instead of re-resolving from `providerSlug` alone.
- `addDefiLlamaAction` now validates exact token-route candidates directly (token page + token-icons gecko URL + token-icon source type) and records explicit failure diagnostics when candidate fields are missing/invalid.
- Resolver fallback now uses full alias context (`buildProviderAliasSet`) with known IDs/aliases rather than `aliases: [requestedSlug]`, improving IO.NET / DIMO / Render token-route parity.
- Token-route persistence now preserves token-source metadata (`defillamaV3=token-icon`, `sourceType=token-icon`, `sourceOrigin=defillama-token-route`, `validatedForTarget=true`) so canonical DefiLlama state reaches REVIEW/OK consistently.
- DefiLlama preview UI now labels token-route recommendations as `token` rather than `protocol` when token-route URL patterns are present.
...
- Onchain Visuals is a simple, premium, share-ready DeFi market card maker.
...
- Onchain Visuals is not an AI product, not a paid SaaS right now and not a crypto data terminal.
...
- Brand Settings now apply to the public homepage hero/header copy, metadata title/description, optional favicon/apple-touch-icon URLs and ShareCard footer/watermark text. If the database is unavailable, the public site falls back to the Onchain Visuals defaults and does not crash.
...
- `/admin/brand` stores brand text and asset URL candidates in `admin_settings`; as of v0.10.1, explicitly saved values feed public copy, metadata, optional favicon URLs and card footer text while defaults remain Onchain Visuals when DB settings are absent.
